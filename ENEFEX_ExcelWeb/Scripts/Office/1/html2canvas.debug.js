...
(function (Html2canvas) {
	...
	Html2canvas.CanvasRenderer = function (width, height) {
		...
		this.taintcontext = this.document.createElement("canvas").getContext("2d");
	...
	Html2canvas.CanvasRenderer.prototype.setFillStyle = function (fillStyle) {
		this.context.fillStyle = typeof (fillStyle) === "object" && !!fillStyle.isColor ? fillStyle.toString() : fillStyle;
		return this.context;
	};
	...
	Html2canvas.CanvasRenderer.prototype.circle = function (left, top, size, color) {
		...
		this.context.arc(left + size / 2, top + size / 2, size / 2, 0, Math.PI * 2, true);
	...
	Html2canvas.CanvasRenderer.prototype.taints = function (imageContainer) {
		if (imageContainer.tainted === null) {
			this.taintcontext.drawImage(imageContainer.image, 0, 0);
			try {
				this.taintcontext.getImageData(0, 0, 1, 1);
				imageContainer.tainted = false;
			}
			catch (e) {
				this.taintcontext = document.createElement("canvas").getContext("2d");
	...
	Html2canvas.CanvasRenderer.prototype.drawImage = function (imageContainer, sx, sy, sw, sh, dx, dy, dw, dh) {
		if (!this.taints(imageContainer) || this.options.allowTaint) {
			this.context.drawImage(imageContainer.image, sx, sy, sw, sh, dx, dy, dw, dh);
		}
	};
	...
	Html2canvas.CanvasRenderer.prototype.shape = function (shape) {
		...
		shape.forEach(function (point, index) {
			if (point[0] === "rect") {
				this.context.rect.apply(this.context, point.slice(1));
			}
			else {
				this.context[(index === 0) ? "moveTo" : point[0] + "To"].apply(this.context, point.slice(1));
			}
		}, this);
		this.context.closePath();
		return this.context;
	...
	Html2canvas.CanvasRenderer.prototype.setTransform = function (transform) {
		this.context.translate(transform.origin[0], transform.origin[1]);
		this.context.transform.apply(this.context, transform.matrix);
		this.context.translate(-transform.origin[0], -transform.origin[1]);
	};
	Html2canvas.CanvasRenderer.prototype.setVariable = function (property, value) {
		if (this.variables[property] !== value) {
			this.variables[property] = this.context[property] = value;
		}
		return this;
	};
	Html2canvas.CanvasRenderer.prototype.text = function (text, left, bottom) {
		this.context.fillText(text, left, bottom);
	};
	...
	Html2canvas.CanvasRenderer.prototype.renderBackgroundRepeat = function (imageContainer, backgroundPosition, size, bounds, borderLeft, borderTop) {
		...
		this.setFillStyle(this.context.createPattern(this.resizeImage(imageContainer, size), "repeat"));
	...
	Html2canvas.CanvasRenderer.prototype.renderBackgroundGradient = function (gradientImage, bounds) {
		if (gradientImage instanceof LinearGradientContainer) {
			var gradient = this.context.createLinearGradient(bounds.left + bounds.width * gradientImage.x0, bounds.top + bounds.height * gradientImage.y0, bounds.left + bounds.width * gradientImage.x1, bounds.top + bounds.height * gradientImage.y1);
